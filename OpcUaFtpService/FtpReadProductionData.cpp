...
int FtpReadProductionData::InitFtpConnection(LPCWSTR iServerAdress, LPCWSTR iServerUsername, LPCWSTR iServerPassword, LPCWSTR iFileServerPath, LPCWSTR iArchDestPath, LoggingToTextFile& MessageFile)
{
	...
	if (hInternet == NULL)
	{
		...
		MessageFile.AppendLog("Internet opening failed: " + to_string(GetLastError()), M_ERROR);
		return GetLastError();
	}
	else
	{
		//cout << "\nInternet connection opened succesfully: ";
		MessageFile.AppendLog("Internet opening succesfull", M_INFO);
...
{
	if (FoundFile[0] == 'D' && FoundFile[1] == 'R' && FoundFile[2] == '_' && FoundFile[15] == '.' && FoundFile[16] == 'c' && FoundFile[17] == 's' && FoundFile[18] == 'v')
	{
		return 1;
	}
	else
	...
	while (getline(file, line)) 
	{
		if (LineNumber == 1)
		{
			...
			while(getline(s, column, ';')) 
			{
				...
				if(ColumnIndex == 0)
					Data.ExampleInt1 = stoi(column);
				else if(ColumnIndex == 1)
					Data.ExampleString1 = column;
				else if (ColumnIndex == 2)
					Data.ExampleString2 = column;
				else if (ColumnIndex == 3)
					Data.ExampleString3 = column;
